...
export default function DemandeDetailPage() {
  ...
  const [isAdmin, setIsAdmin] = useState(false); // ✅ rôle
  ...
  const getQualiteColor = (qualite: string) => ({ bonne: '#be1526', moyenne: '#ff0921', mauvaise: '#d66064' }[qualite] || '#6b7280');
  const getQualiteLabel = (qualite: string) => ({ bonne: 'Très bonne', moyenne: 'Bonne', mauvaise: 'Limitée' }[qualite] || qualite);
          ...
          <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-lg border border-gray-100 max-h-[80vh] flex flex-col">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-gray-400 text-xs font-medium mt-1">Choisissez une de vos cartes à lier à cette demande</p>
              </div>
              ...
              {cartesLoading ? (
                ...
                cartesFiltrees.map(carte => (
                  <div key={carte.id}
                    ...
                    <button onClick={() => handleAssocierCarte(carte.id, carte.nom)}
