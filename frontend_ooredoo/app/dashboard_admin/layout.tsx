...
import {
  ...
  Send, X, MessageSquare, Cpu, ShieldCheck,
  List, CheckSquare
} from 'lucide-react';
            ...
            {messages.length === 0 && (
              <div className="text-center mt-12 space-y-2">
                ...
                <p className="text-gray-400 text-xs px-4 font-medium leading-relaxed">
                  Une question sur la plateforme ? Je suis là pour vous aider.
                </p>
              </div>
            )}
    ...
    <div className="flex h-screen bg-white overflow-hidden font-sans">
      ...
      <aside className={`bg-white border-r border-gray-100 flex flex-col flex-shrink-0 shadow-sm transition-all duration-300 relative z-40 ${
        ...
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
          ...
          <SidebarItem href="/dashboard_admin/demandes" icon={<FileText size={16} />} label="Demander une carte" active={pathname === '/dashboard_admin/demandes'} collapsed={collapsed} />
          <SidebarItem href="/dashboard_admin/demandes/liste" icon={<List size={16} />} label="Liste des demandes" active={pathname.startsWith('/dashboard_admin/demandes/liste')} collapsed={collapsed} />
          <div className="my-3 border-t border-gray-100 mx-2" />
          <SidebarItem href="/dashboard_admin/utilisateurs" icon={<Users size={16} />} label="Ingénieurs" active={pathname.startsWith('/dashboard_admin/utilisateurs')} collapsed={collapsed} />
        ...
        <header className="h-20 bg-white border-b border-gray-100 px-8 flex items-center justify-between flex-shrink-0 z-30 shadow-sm">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2.5 bg-gray-50 px-3 py-1.5 rounded-xl border border-gray-100">
              ...
              <span className="text-xs font-semibold text-gray-700">{userName.split(' ')[0]}</span>
            </div>
          </div>
        </header>
