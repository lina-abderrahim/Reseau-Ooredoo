...
import { Map, PlusCircle, Clock, CheckCircle, XCircle, Calendar, ArrowRight, Search, Globe } from 'lucide-react';
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium mt-1">
            <span className="text-red-600 font-bold">{filteredCartes.length}</span> projet(s) trouvé(s)
          </p>
        </div>
      ...
      {filteredCartes.length === 0 ? (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          ...
          <h3 className="text-xl font-black text-gray-800 mb-2">
            {searchTerm || filter !== 'tous' ? 'Aucune carte trouvée' : 'Votre inventaire est vide'}
          </h3>
          <p className="text-gray-400 max-w-xs mx-auto mb-8 font-medium">
            {searchTerm || filter !== 'tous' ? 'Essayez avec un autre filtre ou terme de recherche.' : 'Commencez à modéliser le réseau en créant votre première carte de couverture.'}
          </p>
